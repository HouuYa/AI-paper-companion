# Attention Is All You Need (Vaswani et al., 2017) - Reviewed
...
## 4. 학습자의 생각 및 질문 (Critical Review & Personal Notes)

*   **나의 생각**: RNN의 순차적 특성을 배제했기 때문에 단어의 순서 정보를 학습하기 위해 Positional Encoding(위치 인코딩)을 더해준 아이디어가 매우 흥미로움. 만약 위치 인코딩이 없다면 Transformer는 단순히 '단어 보따리(Bag-of-Words)'처럼 작동하여 문법 구조를 학습할 수 없을 것임.
*   **비판적 질문**: 긴 문장(Long Context)을 처리할 때 셀프 어텐션의 복잡도가 $O(N^2)$로 증가하는 문제는 어떻게 개선할 수 있을까? 최근 Linear Attention이나 Sparse Attention 기법들이 연구되고 있는 배경을 이해하게 되었음.
...
*   **Q1. Transformer 아키텍처가 기존 RNN 모델에 비해 병렬 처리에 유리한 결정적 이유는 무엇인가?**
    *   *나의 답변*: 순환(Recurrent) 단계를 없애고 모든 토큰의 연관도를 동시에 계산하기 때문이다. (정답)
*   **Q2. 순환 구조를 완전히 없앰으로써 생기는 단어의 순서 정보 누락 문제는 어떻게 보완하였는가?**
    *   *나의 답변*: Positional Encoding(위치 인코딩) 벡터를 입력 임베딩에 더해주어 상대적인 위치 차이를 알게 함. (정답)
